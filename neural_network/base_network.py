...
class NeuralNetwork(object):
	...
	def train_batch(self, feed_dict={}):
		_, loss, acc, summary = self._session.run([self._optimizer, self._loss, self._accuracy, self._summaries], feed_dict=feed_dict)
		self._summary_train_writer.add_summary(summary, global_step=self.global_step)
		self.global_step += 1
		return loss, acc
